...
'''
==========================================================================
 Filename: categorize2.py
 Version: 0.22
 Date: 2026-04-16 20:00 CEST
 Description: Blazing fast memory-heavy categorizer for IPs and Domains.
              Utilizes bulk reads, fast-path skipping, explicitly handles
              BOGONs/RFCs. Supports sectioned output or flattened inline
              comments via the -i/--inline parameter.
...
def categorize_domain(domain):
    ...
    if domain.startswith("wpad.") or domain == "wpad": return "Domain: Web Proxy Auto-Discovery (WPAD)"
...
def main():
    parser = argparse.ArgumentParser(description="Categorize IPs, CIDRs, and Domains (Fast Variant).")
    ...
    for line in raw_lines:
        ...
        if is_fast_ip(token) or token.lower().endswith('.arpa'):
            try:
    ...
    if args.inline:
        ...
        for _, _, item, cat in ip_objs:
            out_append(f"{item} # {cat}")
        for item, cat in domain_items:
            out_append(f"{item} # {cat}")
    else:
        out_keys = sorted([k for k in categories.keys() if k != "Unknown / Undetermined"])
        if "Unknown / Undetermined" in categories:
            out_keys.append("Unknown / Undetermined")
