...
def is_fast_ip(token):
    """Heuristic fast-path validation."""
    if not token: return False
    c = token[0]
    return c.isdigit() or ':' in c or c == '-'

def read_ips_bulk(source, is_verbose, strict):
    """Reads lines natively in bulk and parses networks (supports CIDR, netmask, ranges, and firewall syntax)."""
    networks = []
...
def main():
    ...
    parser.add_argument("-s", "--strict", action="store_true", help="Reject CIDRs with dirty host bits")
    ...
    for bl_source in args.blocklist:
        try:
            raw_blocks.extend(read_ips_bulk(bl_source, v, args.strict))
    ...
    if args.allowlist:
        ...
        for al_source in args.allowlist:
            try:
                raw_allows.extend(read_ips_bulk(al_source, v, args.strict))
            except Exception as e:
